...
\begin{frame}{Overflow}
    \begin{itemize}
        ...
        \item int: hasta $2^{31}-1 = 2.147.483.647$. Algo más de dos mil milllones.
...
\begin{frame}{Funciones clave (C++)}
    \begin{itemize}
        ...
        \item \texttt{lower\_bound , upper\_bound, equal\_range} (algorithm) [begin, end, val]
...
\begin{frame}{Testing por fragmentos}
    \begin{itemize}
        ...
        \item Si testeando $f$ encontramos un caso donde falla, ya sabemos que el bug es  en la $f$: con un caso que falla para todo el programa, no sabríamos dónde está el bug.
    \end{itemize}
\end{frame}
...
\begin{frame}{TDD sobre los fallos}
    \begin{itemize}
        \item Al encontrar un bug en el código o en la idea, que sabemos como solucionar, es conveniente buscar y escribir un caso de prueba donde el programa falle, \textbf{antes} de solucionar el bug.
...
\begin{frame}{TreeSet}
    \begin{itemize}
        \item set<int> en C++
        \item TreeSet<Integer> en Java
        \item no hay (según versión) pero se puede usar collections.OrderedDict
        \item Permiten insertar, borrar, consultar pertenencia y hacer lower\_bound o upper\_bound en $O(\lg N)$
    \end{itemize}
\end{frame}
...
\begin{frame}{TreeMap}
    \begin{itemize}
        ...
        \item TreeMap<Integer,Integer> en Java
        \item collections.OrderedDict en Python
        \item Permiten insertar, borrar, consultar pertenencia y hacer lower\_bound o upper\_bound en $O(\lg N)$
...
\subsection{Movimiento de bloques}
    \begin{frame}{Movimiento de bloques}
        \begin{itemize}
            ...
            \item Puede haber mas de un bloque en la misma casilla en cualquier momento.
