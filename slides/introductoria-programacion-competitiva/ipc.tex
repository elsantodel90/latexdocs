...
\date[TC 2021 - Virtual] % (optional, should be abbreviation of conference name)
{Training Camp 2021}
...
\subsection{Macros y debugging (C++)}

    \begin{frame}{Macro DBG (C++)}
        \begin{itemize}
            \item \#define DBG(x) cerr <{}< \#x <{}< `` = '' <{}< (x) <{}< endl
            
            \ 
            
            \item \#define RAYA cerr <{}< ``========================='' <{}< endl
            
            \ 
            
            \item \#include <cassert>\\
            assert(x > 0); // Llenar el programa de asserts!
            
            \ 
            
            \item Usar \texttt{gdb} para saber la línea de runtime error
        \end{itemize}
    \end{frame}
