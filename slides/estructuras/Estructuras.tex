...
\begin{itemize}
    %\invisible<1>
    {
        \item ¿Qué pasa con la raíz?
        %\pause
        \item Esto nos sugiere un algoritmo de divide and conquer...
        %\pause
        \item ¡Pero la complejidad en peor caso resulta cuadrática!
    }
\end{itemize}
\end{frame}
...
\begin{frame}{Buscando ideas...}
        ...
        \begin{itemize}
            \item Encontrar el mínimo elemento en la heap (raíz del arbol). La solución trivial es $O(n)$
            \item Dividir a los nodos en los menores que la raíz y los mayores que la raíz.  La solución trivial es $O(n)$
        \end{itemize}
    %\invisible<1>
    {
        \begin{itemize}
            \item Idea 1: Para dividir los nodos rápidamente, podemos ordenarlos una sola vez al comienzo. $O(n \lg n)$
            %\pause
...
\begin{frame}{Pero necesitamos más ideas...}
        \begin{itemize}
            \item Idea 2: Para encontrar el mínimo, podemos usar una estructura de RMQ sobre el arreglo de elementos ordenados, ya que siempre se consultará por un intervalo del mismo.
            %\pause
            \item Como no interesa modificar el arreglo, lo mejor es utilizar sparse table ya que es más simple que un segment tree. Luego de un
                primer preproceso $O(n \lg n)$, podremos encontrar mínimos en $O(1)$
            %\pause
...
\begin{frame}{Nosotros ya estamos pero...}
        \begin{itemize}
            \item Para el problema de competencia en cuestión, una solución $O(n \lg n)$ es más que suficiente.
            %\pause
...
\begin{frame}{Visión del usuario}
  \begin{itemize}
    \item Ya hemos mencionado la visión del usuario de esta estructura.
    \item La diferencia consiste en este caso en que además de consultar suma de subarreglos consecutivos, nos interesara modificar
    los elementos almacenados en la estructura. Por simplicidad, supondremos que lo haremos sumándole una constante (negativa o positiva),
    a un valor en una posición dada (Operación que notaremos $add(i,x)$).
    %\pause
    \item Tambien por simplicidad estudiaremos el cálculo de la suma de los elementos con índice menor que un cierto valor $i$ dado,
    operación que notaremos $accum(i)$. Notar que la suma del intervalo $[i,j)$ se calcula simplemente como $accum(j) - accum(i)$.
    %\pause
...
\begin{frame}{Extras}
    ...
    \begin{itemize}
        \item Dado un árbol con raíz, dar una estructura capaz de responder, para cualquier nodo $v$, cuál es el $k$-ésimo nodo en el camino 
        desde $v$ hasta la raíz, en tiempo $O(\lg k)$, con tiempo de preproceso $O(n \lg n)$. En varios problemas se combina esta estructura con
        LCA.
        \item Sumar una constante a todos los elementos de un sub-arreglo de un arreglo en $O(1)$, con la limitación de que solo se puede mirar
        el estado del arreglo al final de todas estas sumas, luego de hacer un cómputo de complejidad lineal. Está fuertemente relacionado con
        las tablas aditivas.
    \end{itemize}
\end{frame}
